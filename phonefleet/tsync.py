...
#    phonelist = connect.scan()
    phonelist = range(10)#[0,1,3,4,5]
    print(phonelist)

    for i in range(10):
        t1 = time.time()
        results = run(phonelist,n=50,iter=10)
        #function to write the Dts in a file, with following lines:
        t2 = time.time()
        print(f'elapsed time :{np.round(t2-t1,decimals=2)} s')
        print(results)

        savefolder = '/home/turbots/Documents/Bicwin2024/git/icewave/icewave/phonefleet/phonefleet/Bic25/Tsync/' 
